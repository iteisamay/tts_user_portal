...
const NewsPlaylist = ({ id, language, description, title, audioUrl, thumbnail, duration: apiDuration }) => {
    ...
    const checkAudioExists = async (id) => {
        try {
            const res = await fetch(`http://10.10.20.199:5050/s1/api/v1/tts/get/${id}`);
        ...
        <div className="relative max-w-sm mx-auto">
            <div className="bg-white rounded-xl p-3 sm:p-4 -mt-2 overflow-hidden">
                ...
                {audioUrl && (
                    <audio
                        ref={audioRef}
                        src={`${process.env.NEXT_PUBLIC_BACKEND_ENDPOINT}/audio/${audioUrl}`}
                ...
                >
                    {thumbnail && (
                        <img
                            src={`${process.env.NEXT_PUBLIC_BACKEND_ENDPOINT}/images/${thumbnail}`}
